...
if (!workerPath || !inputPath) {
  console.error("usage: node probe-cmg-direct-call.mjs cmg.worker.js in.ts [live-chain|vod-chain|live0..live8|vod0..vod8|update] [mediaTagId] [pageHost] [maxBytes] [catch|skip]");
  process.exit(1);
}
...
const originalNal = firstVideoNal(fs.readFileSync(inputPath));
const data = +maxBytes > 0 ? originalNal.subarray(0, +maxBytes) : originalNal;
const host = Buffer.from(pageHost);
log("input", { length: data.length, head: Array.from(data.subarray(0, 32)) });
